...
def get_report(password, reportUri=None):
    ...
    try:
        ...
        repl = Replicon(userid='tweidman', password=password,
                        project_cc=None,
                        expenseSlug=None,
                        description=None,
                        isExpense=False)
...
def get_expense_values(password):
    ...
    list_report = get_report(password=password,
                             reportUri=reportUri)
    ...
    while True:
        ...
        if int(list_report[0][0]) == xw.Range("A" + str(row)).value and list_report[0][1].__eq__(
                str(xw.Range("B" + str(row)).value)):
            break
...
def get_resource_values(password):
    ...
    while True:
        ...
        if xw.Range("B" + str(row)).value is None:
            break
        # Finds the mininum week of the report
        if int(list_report[0][0]) == xw.Range("B" + str(row)).value and list_report[0][1].__eq__(
                str(xw.Range("C" + str(row)).value)):
            break
