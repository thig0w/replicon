...
class Replicon:
    def __init__(self, company='logicinfo', userid='tweidman', password=None, project_cc=2058, expenseSlug=None,
                 description='Created by TW: Change this desc', isExpense=True):
        ...
        data = {
            "companyKey": self.company,
            "loginName": self.userid,
            "targetUrl": ""
        }
        ...
        if isExpense:
            ...
            if self.expenseSlug is not None:
                ...
                data = {
                    "expenseSheetSlug": self.expenseSlug
                }
                jsonresponse = self.__post_to_url(url, data)
                self.expenseUri = jsonresponse["d"]

                # Get expense sheet status
                url = self.serviceUrl + 'ExpenseApprovalService1.svc/GetExpenseSheetApprovalDetails'
                data = {
                    "expenseUri": self.expenseUri
                }
                jsonresponse = self.__post_to_url(url, data)
    ...
    def get_new_entry(self, entry_desc='taxi', project_cc='2084', date=datetime.today(), expense_code='6', currency='9',
        ...
        bclient = 'urn:replicon:expense-billing-option:bill-to-client' if bill_client == 'Yes' else 'urn:replicon:expense-billing-option:not-billed'
        remp = 'urn:replicon:expense-reimbursement-option:reimburse-employee' if reimburse_emp == 'Yes' else 'urn:replicon:expense-reimbursement-option:not-reimbursed'
        recpt = {
            "target": None,
            "image": {
                "base64ImageData": base64_file,
                "mimeType": mime_type
            }
        } if base64_file is not None else None
        ...
        entry = {
            ...
            "project": {
                "uri": "%s" % (self.project_uri)
            },
            "expenseCode": {
                "uri": "urn:replicon-tenant:logicinfo-ref:expense-code:%s" % (expense_code)
            },
            "flatAmountEntry": {
                "incurredAmountNet": {
                    "amount": "%s" % (amount),
    ...
    def set_project_uri_from_cc(self, project_cc=2058):
        ...
        for project in jsonresponse["d"]:
            if project["displayText"].__contains__(str(project_cc)):
                logger.debug('set_project_uri: %s / %s / %s ', project_cc, project["displayText"], project["uri"])
                break
